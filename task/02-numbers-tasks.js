...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  let vComp = x1*x2 + y1*y2;
  let vMod1 = Math.sqrt(x1*x1 + y1*y1);
  let vMod2 = Math.sqrt(x2*x2 + y2*y2);
  return Math.acos(vComp / (vMod1 * vMod2));
}
...
function toNumber(value, def) {
	if (isNaN(value) || value === null || value === undefined) return def;
